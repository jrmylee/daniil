...
class GatedCNN(object):
    ...
    def __call__(self, xW, layer_idx):
        ...
        if self.v_map is not None:
            xW = concatenate([xW, self.v_map], mode='sum', name='h_merge_v_'+str(layer_idx))
        ...
        res = concatenate([xW_f, xW_g], mode='mul', name=stack_tag+'_merge_gate_'+str(layer_idx))
...
class PixelCNN(object):
    ...
    def _build_layers(self, x, h=None):
        ...
        for i in range(1, self.nb_pixelcnn_layers):
            ...
            ### residual connection
            h_stack_out = concatenate([h_stack_out, h_stack_out_prev], mode='sum', name='h_residual_'+str(i))
